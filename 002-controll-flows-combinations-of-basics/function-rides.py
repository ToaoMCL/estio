...
def evaluate(height, park):
    ...
    if height >= white_knuckle_height_requirement and park >= permited_parks[0]:
        return "You can use all rides."
    elif height >= family_height_requirement and  park >= permited_parks[1]:
        return "You can use the kids & family rides."
    elif height >= kids_height_requirement and park >= permited_parks[2]:
        return "You can only use the kids rides."
    else:
        return "You're not high enough to use any rides. Or chosen park is not in our permited list"
